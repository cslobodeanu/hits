...
#define _ANGLE(a)		Mathf::ToRadians(a)
...
{
	const float c = cosf(_ANGLE(angle));	const float s = sinf(_ANGLE(angle));	const float C = 1.f - c;
...
{
	const float s = sinf(_ANGLE(angle));
	const float c = cosf(_ANGLE(angle));
...
{
	const float halfFov = _ANGLE(fieldOfView) * 0.5f;
...
Matrix	Matrix::CreatePerspectiveObliqueClipPlane(const Matrix *const projection, const Matrix *const view, const Plane *const clipPlaneWorldSpace)
{
	...
	const float norm = Mathf::Abs(clipPlane.normal.z);
...
{
#ifdef DIRECTX_PORTZZ
	Matrix suffix;
	Plane clipPlane;

	Plane pp = *clipPlaneWorldSpace;
	pp.Normalize();
	...
	Plane::Transform(&pp, result, &clipPlane);
	...
	if(clipPlane.d > 0)
	{
		Plane negp(-clipPlane.normal.x, -clipPlane.normal.y, -clipPlane.normal.z, -clipPlane.d);
		Plane::Transform(&negp, result, &clipPlane);
	}
...
Matrix Matrix::CreateFromDirectionTranslationScale(const Vector3 *const direction, const Vector3*const pos, const Vector3 *const scale)
{
	Matrix ret;
	Matrix::CreateFromDirection(direction, &ret);
	ret.Scale(scale);
	ret.Translation() = *pos;
	return ret;
}

void Matrix::CreateFromDirectionTranslationScale(const Vector3 *const direction, const Vector3*const pos, const Vector3 *const scale, Matrix *const result)
{
	Matrix::CreateFromDirection(direction, result);
	result->Scale(scale);
	result->Translation() = *pos;
}
...
{
	const float cp = Mathf::Sqrt(1.f - (direction->y*direction->y));					//cos pitch
	const float minus_rnorm = -Mathf::Rsqrt((direction->x*direction->x) + (direction->z*direction->z));
...
void Matrix::MoveDepthTo01( Matrix *const mat )
{
...
void Matrix::MoveDepthTo01( const Matrix *const mat, Matrix *const result )
{
	result->m11 = mat->m11;
	result->m12 = mat->m12;
	result->m13 = mat->m13;
	...
	result->m31 = 0.5f*mat->m31 + 0.5f*mat->m41;
	result->m32 = 0.5f*mat->m32 + 0.5f*mat->m42;
	result->m33 = 0.5f*mat->m33 + 0.5f*mat->m43;
	result->m34 = 0.5f*mat->m34 + 0.5f*mat->m44;
...
const Vector3 Matrix::GetScale() const
{
	...
	const Vector3& u = Up();
	const Vector3& f = Backward();
	const float rl = r.Length();
	const float ul = u.Length();
	const float fl = f.Length();
	return Vector3( rl, ul, fl );
}

Vector3 Matrix::operator * (Vector3 p) const
{
	return Vector3::Transform(&p, this);
}

Matrix Matrix::operator * (Matrix m) const
{
	Matrix tmp;
	Matrix::Multiply(this, &m, &tmp);
	return tmp;
}
