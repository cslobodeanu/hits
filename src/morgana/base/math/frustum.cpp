...
float Frustum::Project2DZDist(const Vector3 *const p, Vector2 *const pout) const
{
...
float Frustum::Project2DZDist(const Vector3 *const p, Vector3 *const pout) const
{
...
void Frustum::Update(const Matrix *const ProjectionMat, const Matrix *const ViewMat)
{
...
bool Frustum::Contains(const Vector3& point, const float radius) const
{
...
bool Frustum::Contains(const Box *	const box) const
{	
...
bool Frustum::Contains(const Box *	const box, const Matrix *const boxTransform) const
{
	NOT_IMPLEMENTED;
	return true;
}
...
{
	Matrix viewProj;
	Matrix::Invert(&ProjectionViewMat, &viewProj);
	
	Vector3 min = Vector3::One * Mathf::MaxFloat;
	Vector3 max = -min;
	for (Vector3* v = corners11; v < corners11 + 8; v++)
	{
		Vector3 tmp;
		Vector3::TransformPerspectiveDivide(v, &viewProj, &tmp);
		min = Vector3::Min(min, tmp);
		max = Vector3::Max(max, tmp);
	}

	AABB = Box::FromMinMax(min, max);
